...
class TestInvalidWebsocketResponse:
    def test_unexpected_response(self, proto_ver, proto_name, fake_websocket_broker):
        ...
        with fake_websocket_broker.serve(WebsocketHandler), pytest.raises(
...
class TestBadWebsocketHeaders:
    ...
    def test_no_upgrade(
        self, proto_ver, proto_name, fake_websocket_broker, init_response_headers
    ):
        ...
        with fake_websocket_broker.serve(response), pytest.raises(
            WebsocketConnectionError
        ) as exc:
            mqttc.connect("localhost", fake_websocket_broker.port, keepalive=10)
...
class TestValidHeaders:
    ...
    def _get_callback_handler(self, response_headers, check_request=None):
        ...
        class WebsocketHandler(socketserver.BaseRequestHandler):
            def handle(_self):
                ...
                key = re.search(
                    "sec-websocket-key: ([A-Za-z0-9+/=]*)", decoded, re.IGNORECASE
                ).group(1)
    ...
    def test_correct_path(
        ...
        def check_path_correct(decoded):
            ...
            if mqtt_path:
                assert (
                    re.search(f"GET {mqtt_path} HTTP/1.1", decoded, re.IGNORECASE)
                    is not None
                )
