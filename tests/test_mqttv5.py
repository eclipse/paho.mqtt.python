...
class Callbacks:
    ...
    def __str__(self):
        return (
            str(self.messages)
            + str(self.messagedicts)
            + str(self.publisheds)
            + str(self.subscribeds)
            + str(self.unsubscribeds)
            + str(self.disconnects)
        )
...
def cleanRetained(port):
    ...
    curclient = paho.mqtt.client.Client(
        b"clean retained", protocol=paho.mqtt.client.MQTTv5
    )
...
def cleanup(port):
    ...
    for clientid in clientids:
        curclient = paho.mqtt.client.Client(
            clientid.encode("utf-8"), protocol=paho.mqtt.client.MQTTv5
        )
        curclient.loop_start()
...
class Test(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with unittest.mock.patch("signal.signal", unittest.mock.MagicMock()):
            ...
            cls._test_broker_port = (
                mqtt.brokers.listeners.TCPListeners.server.socket.getsockname()[1]
            )
        setData()
    ...
    def test_retained_message(self):
        ...
        response = callback.wait_connected()
        aclient.publish(
            topics[1], b"qos 0", 0, retain=True, properties=publish_properties
        )
        aclient.publish(
            topics[2], b"qos 1", 1, retain=True, properties=publish_properties
        )
        aclient.publish(
            topics[3], b"qos 2", 2, retain=True, properties=publish_properties
        )
        ...
        self.assertTrue(
            userprops in [[("a", "2"), ("c", "3")], [("c", "3"), ("a", "2")]], userprops
        )
    ...
    def test_zero_length_clientid(self):
        ...
        client0.connect(
            host="localhost", port=self._test_broker_port, clean_start=False
        )
    ...
    def test_offline_message_queueing(self):
        ...
        logging.info(
            "This server %s queueing QoS 0 messages for offline clients"
    ...
    def test_overlapping_subscriptions(self):
        ...
        oclient.subscribe(
            [
                (wildtopics[6], SubscribeOptions(qos=2)),
        ...
        if len(ocallback.messages) == 1:
            ...
            self.assertEqual(
                ocallback.messages[0]["message"].qos,
                2,
                ocallback.messages[0]["message"].qos,
            )
        else:
            ...
            self.assertTrue(
                (
                    ocallback.messages[0]["message"].qos == 2
                    and ocallback.messages[1]["message"].qos == 1
                )
                or (
                    ocallback.messages[0]["message"].qos == 1
                    and ocallback.messages[1]["message"].qos == 2
                ),
                callback.messages,
            )
        oclient.disconnect()
    ...
    def test_subscribe_failure(self):
        ...
        self.assertEqual(
            response["reasonCodes"][0].getName(),
            "Unspecified error",
    ...
    def test_session_expiry(self):
        ...
        fclient.connect_async(
            host="localhost",
            port=self._test_broker_port,
            clean_start=False,
    ...
    def test_message_expiry(self):
        ...
        laclient.publish(
            topics[0], b"qos 1 - expire", 1, retain=False, properties=publish_properties
        )
        laclient.publish(
            topics[0], b"qos 2 - expire", 2, retain=False, properties=publish_properties
        )
        ...
        self.assertTrue(
            lbcallback.messages[0]["message"].properties.MessageExpiryInterval < 6,
            lbcallback.messages[0]["message"].properties.MessageExpiryInterval,
        )
        self.assertTrue(
            lbcallback.messages[1]["message"].properties.MessageExpiryInterval < 6,
    ...
    def test_subscribe_options(self):
        ...
        lacallback.wait_connected()
        laclient.subscribe(
            topics[0], options=SubscribeOptions(qos=2, retainAsPublished=True)
        )
        lacallback.wait_subscribed()
    ...
    def test_subscription_identifiers(self):
        ...
        self.assertEqual(
            lacallback.messages[0]["message"].properties.SubscriptionIdentifier[0],
            456789,
            lacallback.messages[0]["message"].properties.SubscriptionIdentifier,
        )
        laclient.disconnect()
        lacallback.wait_disconnected()
        laclient.loop_stop()
    ...
    def test_request_response(self):
        ...
        self.assertEqual(
            lbcallback.messages[0]["message"].properties.CorrelationData,
            b"334",
            lbcallback.messages[0]["message"].properties,
        )
    ...
    def test_server_topic_alias(self):
        ...
        self.assertTrue(
            hasattr(lacallback.messages[0]["message"].properties, "TopicAlias"),
            lacallback.messages[0]["message"].properties,
        )
        ...
        self.assertFalse(
            hasattr(lacallback.messages[1]["message"].properties, "TopicAlias"),
            lacallback.messages[1]["message"].properties,
        )
        self.assertFalse(
            hasattr(lacallback.messages[2]["message"].properties, "TopicAlias"),
            lacallback.messages[2]["message"].properties,
        )
    ...
    def test_will_delay(self):
        ...
        laclient.will_set(
            topics[0],
            payload=b"test_will_delay will message",
    ...
    def test_shared_subscriptions(self):
        ...
        j = 0
        while (
            len(lacallback.messages) + len(lbcallback.messages) < 2 * count and j < 20
        ):
            time.sleep(0.1)
            j += 1
        time.sleep(1)
