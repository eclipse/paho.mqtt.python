...
def gen_connect(
    ...
    if (proto_ver & 0x7F) == 3 or proto_ver == 0:
        packet = packet + struct.pack(
            "!H6sBBH", len(b"MQIsdp"), b"MQIsdp", proto_ver, connect_flags, keepalive
        )
    elif (proto_ver & 0x7F) == 4 or proto_ver == 5:
        packet = packet + struct.pack(
            "!H4sBBH", len(b"MQTT"), b"MQTT", proto_ver, connect_flags, keepalive
        )
...
def gen_publish(
    ...
    if proto_ver == 5:
        if qos > 0:
            return struct.pack(
                "!B" + str(len(rlpacked)) + "s" + pack_format,
                cmd,
                rlpacked,
                len(topic),
                topic,
...
def _gen_command_with_mid(cmd, mid, proto_ver=4, reason_code=-1, properties=None):
    if proto_ver == 5 and (reason_code != -1 or properties is not None):
        ...
        if properties is None:
            ...
            return struct.pack(
                pack_format, cmd, 2 + 1 + len(properties), mid, reason_code, properties
            )
    else:
...
def gen_subscribe(mid, topic, qos, cmd=130, proto_ver=4, properties=b""):
    ...
    if proto_ver == 5:
        if properties == b"":
            ...
            pack_format = (
                "!H" + str(len(properties)) + "s" + "H" + str(len(topic)) + "sB"
            )
...
def gen_unsubscribe(mid, topic, cmd=162, proto_ver=4, properties=b""):
    ...
    if proto_ver == 5:
        if properties == b"":
            ...
            return struct.pack(
                pack_format, cmd, 2 + 2 + len(topic) + 1, mid, 0, len(topic), topic
            )
        else:
            properties = mqtt5_props.prop_finalise(properties)
            packet = struct.pack("!B", cmd)
            ...
            packet += struct.pack(
                pack_format, mid, len(properties), properties, len(topic), topic
            )
            return packet
    else:
...
def _gen_short(cmd, reason_code=-1, proto_ver=5, properties=None):
    if proto_ver == 5 and (reason_code != -1 or properties is not None):
        ...
        if properties is None:
            ...
            properties = mqtt5_props.prop_finalise(properties)
            return (
                struct.pack("!BBB", cmd, 1 + len(properties), reason_code) + properties
            )
    else:
